...
`Φ(n) ≥ n(S−2P₂) − 2` gives the clean sufficient **criterion**
...
**What remains open — the single lemma `C-B-FIN`:** *the C-B residual
(`≤2`-good, gcd=1, window-relevant, `CRIT ≤ 7/2`) is finite.* Evidence: the saturation
above + the junk-retirement argument (every junk ray self-retires; so the residual is
generated by "essential" cores, whose clustering constraint `ΣD ≤ 2Σgcd + (7/2)·min(D)`
is very rigid). This **supersedes G3′+C4 as the canonical open piece** — same
mathematical content, but the covering theorem is 3 lines (Lean-able in an afternoon)
instead of per-shape floor work, and the bank is 195 sets instead of 22,693.
Honest tier: C-B theorem PROVED; census/bank COMPUTED (dual entries ≤ 120); C-B-FIN

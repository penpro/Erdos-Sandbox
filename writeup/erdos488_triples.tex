...
\texttt{sorry} in the accompanying Lean development; moreover MalekZ (March 2026)
exhibited a family showing the fixed-threshold reduction underlying that step
\emph{fails} once $\min A\ge 3$. So the three-generator case is, in practice,
still open.
...
is left as \texttt{sorry} in the accompanying Lean files, and MalekZ (March 2026)
gave a family showing the fixed-threshold reduction fails for $\min A\ge 3$---so
this is not a cosmetic gap. By contrast, the proof above uses only $B$ itself, no split on

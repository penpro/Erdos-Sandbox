...
| 9 | **Theorem 9** — (★) for every primitive core `|P| ≤ 3`, sorry-free, via charge decomposition `s(n)−2P(n)=X_a+X_b+X_c ≥ 3` ⇒ `2B(n) > nS` | **Sub:attack** (`a8eb4501`, claude-fable-5); audited + written up by Claude-main | **novel-method / independent-verification of result** — RESULT publicly *claimed* by Chojecki Cor 4.7 (Lean modulo one `sorry`; community treats `|A|≥3` open); our route is a different, elementary, sorry-free proof | `tmp/wf_result_a8eb4501afb88b28f.md`; `triples_writeup.md`; chat *Claude "PROVED … size ≤ 3"* (tag `PROVED`/`NOVEL?`) |
